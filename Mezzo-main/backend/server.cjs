...
if (TAK_CONFIG.enabled) {
  ...
  class SimpleTAKClient {
    ...
    connect() {
      console.log(`🔌 Connecting to TAK Server: ${this.config.host}:${this.config.port}`);
      ...
      this.socket.on('data', (data) => {
        console.log('📥 Received from TAK Server:', data.toString().substring(0, 100));
        this.handleTakMessage(data.toString());
      });
    ...
    reconnect() {
      ...
      console.log('⏳ Reconnecting to TAK Server in 5 seconds...');
    ...
    sendCoT(cotXml) {
      if (!this.connected || !this.socket) {
        console.warn('⚠️  TAK Server not connected, cannot send CoT');
        return false;
      }

      try {
        this.socket.write(cotXml + '\n');
        console.log('📤 Sent CoT to TAK Server');
        return true;
      } catch (error) {
        console.error('❌ Failed to send CoT:', error.message);
        return false;
      }
    }
  ...
  takClient = new SimpleTAKClient(TAK_CONFIG);
  takClient.connect();
}
